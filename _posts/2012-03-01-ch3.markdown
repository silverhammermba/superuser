...
* There are a bunch of missing byte values: 0x00-0x1F and 0x7F-0xFF. The reason
  for some of these (0x80-0xFF) is because ASCII only uses the first 7 bits of
  each byte so only the first 2<sup>7</sup>=128 byte values can be used. The
  other missing characters are "unprintable ASCII". They include characters
  representing line breaks and indentation or even "control characters" that can
  have special meaning to the program using the ASCII text
...
and we've added `<br/>` tags at the end of each line to indicate line
...
readable. Notice how I use identation to make it clearer where tags start and
end.
{: .note}
...
An attribute goes inside a start tag after the tag name and looks like
`name="value"`. One way you could read this is "Richard Wilbur is an anchor with
an href value of `http://en.wikipedia.org/wiki/Richard_Wilbur`". An anchor's
href describes the Internet address of another HTML document that our HTML
document links to.
...
Pretty cool, huh? To prove that there's no trickery going on here, try right
clicking on that page. The menu that pops up should have an option like "View
page source" (if you're reading this on your phone, you might have to dig around
a bit to find this option). This shows you exactly what HTML your browser is
interpreting to create the page.
...
2. View the page source of _this_ page; it's written in HTML! Try to match up
   ...
   HTML. You can see how I actually write the book [here][gh]{: .alert-link}.
   {: .deeper}

   [gh]: https://github.com/silverhammermba/superuser
