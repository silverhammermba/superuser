...
an href value of http://en.wikipedia.org/wiki/Richard_Wilbur". An anchor's href
describes the Internet address of another HTML document that our HTML document
links to.
...
interpreting to create the page. "Interpreting" is a very important word here
since different people might want to interpret HTML differently. For example a
blind person might use a web browser that interprets HTML into a medium of touch
and sound so that they can still interact with it.
...
3. Find some small text document somewhere: an article in a magazine, a
   restaurant menu, something like that. Try adding HTML markup like we did to
   the quotation. There are a lot more tags to choose from than the few I showed
   you. Check out the full list of HTML tags [here][moz]{: .alert-link}.
</div>
{: exercise}
