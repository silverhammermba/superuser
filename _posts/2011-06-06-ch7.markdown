...
pixel is the exact same shade. To store that image as digital data, you could
simply store each red pixel. As a digital description of the image, that's kind
of like saying "The pixel in the top left corner is red, and so is the one next
to it, and so is the one next to that, and so is... and the leftmost pixel in the
next row is also red, and so is the one next to it, and..." It's a bit silly,
right? Why not just say "All 6 million pixels are red"? This is a good example
of where _compression_ is useful.
...
the data is 00, then we simply list the color of each pixel going from left to
right and top to bottom; if the first byte of the data is 01, then we list just
one color and all pixels of the image are that color. For our all-red image, we
can encode it two different ways using this format: with the first byte being 00
or the first byte being 01. The former would require 6MB as before, but the
latter would require only 4 bytes! That's a huge saving!
...
well. However the are general image compression formats (such as PNG)
...
all? In other words, why not compress the image by removing subtle information
that we can't see i.e. replace similar colors with just one color? Compressing
in this way - by removing unimportant information - is called "lossy"
compression and it usually offers even greater savings of data size than the
previous "lossless" compression. JPEG (or JPG) is a common lossy image
compression format.

![the effects of increased compression]({{ site.baseurl }}/img/flower_comp.png)
{: .pull-left}

However lossy compression does carry a risk. Again, computers do not understand
the content of the images they manipulate so there is no guarantee that a
compression algorithm will always make the right call when it decides what
information is "unimportant". It is usually easy to see when an image has been
poorly compressed due to the presence of visual artifacts such as blurriness or
discoloration. On the left we see the flower image compressed three times with
different degrees of compression (a feature of the JPEG format lets you choose
between the data size and the image quality). The original image takes up
