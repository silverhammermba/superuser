...
that computation is very boring. Since computation relies on
...
causes people to make silly mistakes. Secondly, humans don't have very good
...
book-keeping to bypass our limited memory. Lastly humans aren't terribly fast at
computation. For big companies with lots of accounting requirements, humans are
...
crucial stepping stone to modern computers, so we're going to relive it right
now by inventing a machine that can add two numbers with much less chance of
human fallibility mucking it up. We're going to work our way up to this machine
...
we'll evenly write the digits 0&ndash;9 in order. We can attach a handle to the
wheel for rotating it and draw a mark on the table to point at a digit on the
wheel. Tada! We now have a very simple computing machine! In particular, this is
...
find the digit you want to add to, turn the crank, and read the result. But what
happens if we give it 9 as input? Argh! The wheel circles back around to 0!

![Overflow]({{ site.baseurl }}/img/overflow.png)
{: .pull-left}

We can solve this problem by adding another wheel to our contraption. The second
wheel is exactly like the first, except rather than turning it directly with a
handle we attach it to the first wheel. The attachment should be designed such
that the second wheel remains stationary when we turn the crank. Except when the
first wheel turns from 9 back to 0, then the second wheel advances by one digit
(the diagram below shows a crude example of this, but if you want to see how
this is done for real check out this [video][yt]). Now when we give an input of
09, turning the handle changes the 9 to 0, which advances the 0 to 1 and we get
10! Success!  If we keep turning, the second wheel remains at 1 (giving us 11,
12, 13, ...) until we get to 19 and it once again advances to 20.
...
0 advances the leftmost 9 to 0 as well, so our machine says that 99+1=0. Dang!
We can keep adding more wheels like we did before but that only delays the
...
at the moment to handle the input we have. You might be surprised to know
that&mdash;as simple as our little machine is&mdash;it shares this problem with
every modern computer. It's such a common problem that computer scientists even
have a special term for it: "overflow". As is common in computer science, from
now on we'll just assume that we have lots and lots of wheels&mdash;not
infinitely many, but enough that we can handle reasonably large numbers without
overflowing.
...
So 00 is a sensible output from the machine. Do a few examples and you'll
see that this always works:

100 + 1 = 101; 101 &divide; 100 = 1 with remainder 1

101 + 1 = 102; 102 &divide; 100 = 1 with remainder 2
...
which turns the wheel to its left from 0 to 9, until every wheel is set to 9
(giving the largest number the machine can display). Again, this problem is
unavoidable.
