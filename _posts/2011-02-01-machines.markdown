...
instructions with no room for interpretation &ndash; it can be very uncreative, dull
...
with computation &ndash; such as the abacus and slide rule &ndash; but these essentially
...
as simple as our little machine is &ndash; it shares this problem with every modern
...
we'll just assume that we have lots and lots of wheels &ndash; not infinitely many,
...
Such a machine would &ndash; in a sense &ndash; be the last computing machine you would ever
...
1. A **tape**, on which we can write a sequence of digits, which serves as the
   ...
   assume that the tape is very long &ndash; long enough for whatever computation we
   are performing. We assume that the tape starts with every position in the
   sequence set to 0
2. A **head** which can read and write digits on the tape &ndash; but
