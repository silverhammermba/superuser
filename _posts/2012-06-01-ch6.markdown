...
Probably the scariest way that malware can get on your computer is via software
vulnerabilities or "exploits". An exploit is when software can be manipulated to
do something that it was never intended to do. In many cases these exploits are
innoculous, but in some cases (especially when the software in question uses the
Internet) the implications can be catastrophic.
