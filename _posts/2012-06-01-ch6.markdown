...
It can get even worse. Have you ever reused your username and password on
...
If you have a really good memory, you might realize that we had a similar
problem [before][cs]: when Alice was sending Bob a book via the worst postal
service ever, we were concerned that the book Bob received might not match the
version Alice sent. In that case, we solved our problem using a checksum: a
small bit of data that acted like a summary of the data being transmitted. If
the summaries didn't match, it was pretty unlikely that the book matched the
original, and vice versa. So maybe a checksum is the tool we're looking for!
When a user chooses their password, we compute a checksum of the password and
store _that_. Then when they access their account later, we again compute the
...
get output is called **hashing** the data. The output of a hash function is also
sometimes called a **hash** or **digest**.
{: .definition}
...
because so many computers users are stupid and lazy: in one of the largest
breaches of an improperly secured password database, hackers found that well
over **2 million users** had "123456", "123456789", or "password" as their
password. That may not be useful information for breaking into a _specific_
person's account, but attackers who just want access to _a lot_ of accounts
(whoever may own them) can usually succeed by programming a computer to try
every user name against the most common known passwords.

There are two main approaches to password guessing:

1. Guessing known passwords. As I just described, this simply tries the common
   passwords already released publicly by hackers
2. Dictionary attacks. A great number of passwords _not_ included in the common
   password lists nonetheless follow very similar patterns. For example, many
   passwords consists of an English word possibly followed by a few digits
   ("password1"). A computer can try every English word in the dictionary
   combined with any combination of 0-3 digits in a matter of seconds. Even if
   you really esoteric words like "imprimatur" provide little protection.

And so we come to the real challenge: how do you make a password that is hard
for a hacker to guess? More importantly, how do you make such a password that is
also easy to remember? In coming up with such passwords, it can be helpful to
ask yourself:
...
and choose a character for each word
    ...
    Joseph's face was black as night and the pale yellow moon shown in his eyes
...
2. One way to determine the strength of a method for creating passwords is
   counting choices. Suppose our password consists of three parts A, B, and C
   where we can substitute different values for each part. If A can be any
   single digit, that's 10 choices. If B can be any English word (counting
   conjugations, etc.), that's about 173,000 choices. If C can be any character
   on the keyboard, that's 95 choices. So to get the total number of choices we
   multiply: 10&times;173,000&times;95=164,350,000.

   Come up with a couple different password generation methods and try to count
   their strength in terms of the number of possible password choices. You may
   have to estimate a few counts. Which methods seem like a good trade-off
   between strength/memorability?
</div>
