...
Now you can explorer deeper and deeper like so:
...
So now we can easily explorer deeper, but is there an easy way to go back? Yep!
...
of lines. The command `tail` is similar but prints the last few lines of each
file.
...
You can scroll up and down and quit using the same keyboard buttons as in `man`.
