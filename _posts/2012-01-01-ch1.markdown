...
<img src="{{ site.baseurl }}/img/simpnet.png" alt='A simple "network"'/>
</div>
...
entire packet, we would expect the checksum to almost always fail - unless we
...
Notice that we no longer specify the size of the packet - every packet is fixed
...
to receive (bytes &div; 14 rounded up), and the number of 00 bytes that Alice
added on to the end of the last packet (remainder of bytes &div; 14). He then has
everything he needs to reassemble the data once he gets all of the packets.

<div class="alert alert-info">
<ol><li>
Both example packet formats in this chapter (in the <a class="alert-link" href="#pak">Packaging</a>
and <a class="alert-link" href="#dis">Disassembly</a> sections) impose
restrictions on the total number of bytes that can be sent.  What are these
limits?
</li><li>
Consider a new situation where Alice and Bob want to have a conversation. Adapt
the example protocol (or design a new one) which will allow them to do so. What
new challenges does this situation pose?
</li></ol>
</div>
