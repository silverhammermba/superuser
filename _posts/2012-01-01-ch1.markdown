...
service know where to look for it?

The solution is to use a <strong>package</strong>. Let's consider how this
solves the problem:
...
* The package removes the need to know what exactly Alice is sending. If it is
  ...
  <em>anything that fits in a package</em> - be it a book, photograph, CD, etc.

The computer equivalent of package provides these same benefits.

<div class="alert alert-warning">
A <strong>packet</strong> is a data format that wraps other data formats. It
provides information such as the amount of data being sent and where it is being
sent to. A protocol must specify what packet format is used to transmit data.
</div>
...
* 4 bytes for the sender
* 4 bytes for the intended recipient
* 4 bytes indicating the length of the next section:
* N bytes of data

<div class="alert alert-danger">
Don't worry about what exactly we mean by "sender" and "recipient" here. For now
just think of them as return address and address, respectively. But they will
come in to play later.
</div>

Then Alice and Bob would use this format for all information exchanged between
them. If Alice wants to send Bob a picture, she first adds these 12 bytes in
front of the image format. Then Bob can receive that information without knowing
anything about the image format Alice used - he gets everything he needs from
the first 12 bytes.
...
loses packages, this exchange could take quite a while - but it's the only way
...
receiving them without issue. But Alice really wants Bob to get that book, it's
...
garbled version of the book, he will reply with an ACK and Alice will be non the
wiser.
...
it is extremely unlikely that the numbers would still match.
...
a book, but with computers we don't have this problem.

Remember that all digital information essentially amounts to numbers, so we can
easily come up with a check that works for any packet - regardless of its
contents. For example, simply interpet each byte of data as a non-negative
integer and add them all up (ignoring overflow).  Alice and Bob can both perform
this check on the packet without knowing what is inside it.

<div class="alert alert-warning">
A <strong>checksum</strong> is a function that takes an arbitrary number of bytes as
input and outputs a fixed number of bytes as output. Usually this function
...
depend on the kind of data being sent. Note that - like counting 'e's - the
checksum doesn't provide absolute certainty that the data are correct. A byte
has 1 of 256 possible values, so if a message is randomly garbled there is a
1/256 (0.39%) chance that the checksum will still match. Those odds seem pretty
good, but real computer communications usually use even more complicated
checksums that have an even smaller chance of missing an error.

Again, if the connection is really bad, this can involves lots and lots of
resends, but it's still worth it. You would be surprised at how much damage a
single erroneous bit can cause to a computer.

### Dissassembly ###

Now let's suppose that our postal service is really unreliable and that there is
high chance of them either delivering the book in an altered state or not at
all. Suppose also that - for some reason - the bigger the book is, the higher
the chance is that they'll mess up.

<div class="alert alert-danger">
Yes, that doesn't make sense in the analogy but that's how it works for
computers. I'll explain later.
</div>

Our protocol lets Alice and Bob know when the book hasn't made it, but if the
book is really long the postal service becomes really unreliable. Suppose
there's a 95% chance of them messing up: statistically, that would end up taking
quite a lot of shipments before Bob gets the book successfully. Furthermore, all
of those resent shipments would seem really wasteful. What if the postal service
only removed the last page? Why resend the entire book?

> How can Alice decrease the amount of redundant shipping required when the postal
> service is really unreliable?

What if Alice sends Bob the book one page at a time - each with its own
packaging and checksum? This provides some nice advantages:

* Each package is smaller, reducing the chance of the postal service messing up
...
* Alice sends each page in its own package
...
To prove that this extra work is worth it, imagine that the Book is 100 pages
long and we expect the postal service to screw up 95% of the time. If we send
the whole book at once, we would expect to need to send the book 20 times before
Bob gets it intact - that's 2000 total pages sent.

If we instead use the newer protocol, we still expect to send each package about
20 times before Bob gets it but now we no longer need to resend pages once Bob
receives them.

TODO explain garbles in terms of book length to show how shorter packages is
worth it
