...
A **secure** key is one with so many possible values that even if you could
guess keys very quickly using a computer, it would take so long to find the
right one that the information you could decrypt with the key would no longer be
useful.
{: .definition}
...
this example, X&times;Y is kind of like a public encryption key and X is like
the private decryption key: as long as we keep X secret, we can share X&times;Y
freely and everyone will have a hard time finding X and Y.
