...
encryption? Luckily there is. In fact there is a _perfect_ cipher - one that is
...
Randomness is sometimes called **entropy** - especially when measuring how
random something is.
...
encryption key can be distributed publicly - it's like the unlocked safe that
...
For example, suppose I ask you to find the two integers X and Y such that
X&times;Y=43,047,409. This is a rather tricky problem: no one knows a faster way
of solving it than just trying every possible value for X and Y until you
stumble on the right ones. Now suppose I tell you that X=10,079 - suddenly the
extremely difficult problem is extremely easy: you can just divide to get Y. In
this example, X&times;Y is kind of like a public key encryption key and X is
like the private decryption key: as long as we keep X secret, we can share
...
4. This substitution cipher has over 4 septillion possible keys - that's greater
