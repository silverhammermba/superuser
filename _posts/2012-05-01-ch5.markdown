...
combination lock or a physical key for a padlock, but with a cipher it's extra
information.
...
is to choose the register size in bits, N, and then choose "taps" between 0 and
N&minus;1.
...
the resulting bit on the end, and shift everything over by one. Let's do an
example.
...
Larger shift registers do even better: with an 18-bit register and taps 0 and
11, we get over 250 thousand psuedorandom bits of key!
...
trial-and-error so quickly. For security, you need a cipher with a key large
enough that trying all possible keys would be prohibitive even with a computer.
At the time of this writing, 80 bits is considered about the bare minimum key
size you need to still be considered secure. Most ciphers play it safe by using
even larger keys. So PRNGs still have a place: by using one with a key that is
just big enough to be secure, you can quickly encrypt plaintext that is many
times larger than your key.
...
* What if Bob loses the key? Now the safe is no longer secure and they need to
  meet in person again to exchange a new key.
...
to lock it - you only need one to unlock it - but for our ciphers we encrypted
...
have two keys: one key for encrypting, one key for decrypting. The encryption
key can be distributed publicly - it's like the unlocked safe that Bob sends in
the mail. The decryption key the recipient keeps secret.
...
For example, what is the product 10,079&times;4,271? Easy (especially with a
computer): 43,047,409. Now suppose I only gave you 43,047,409 and asked you to
find the two numbers that it is a product of. Much harder, right? _But_ if I
then tell you that one of the numbers is 10,079, it's suddenly easy again: you
can divide.

You might have noticed me try to sneak something by you just then: "find **the**
two numbers". How do I know that those are the only two numbers that multiply to
that answer? It's because 10,079 and 4,271 are [prime][prime]. A result of [the
fundamental theorem of arithmetic][tfa] is that the product of two prime numbers
cannot be the product of any other integers (ignoring 1, of course).
{: .deeper}

[prime]: http://en.wikipedia.org/wiki/Prime_number
[tfa]: http://en.wikipedia.org/wiki/Fundamental_theorem_of_arithmetic

So, in this case, 43,047,409 would be the public encryption key and 10,079 (or
4,271) would be the private decryption key.
