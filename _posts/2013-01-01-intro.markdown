...
previous operating procedure &ndash; inputting instructions, turning on the
computer, waiting for output, and turning off the computer &ndash; no longer
...
a computer whenever you want &ndash; even while the computer may be running
...
checks to see if the program is starting to run out of room in it reserved
...
which can be plugged into a computer to extend its capabilities. This could
include a HDD for additional memory, or a monitor for displaying images using
pixels. Each of these peripherals usually worked in slightly different ways even
when performing essentially the same task, and so required different program
instructions in order to operate them. For example one company's monitors might
use a different data format for pixel colors than another company's.

[pci]: {{ site.baseurl }}/part1/hardware/

This was not only a nightmare for computer programmers &ndash; who needed to
consider all of the different instructions needed for making their programs work
with all of the popular peripherals &ndash; it was also a nightmare for the
loader, which might have several programs running at once all trying to use the
same peripherals.

The solution was to modify the loader still further. The loader was extended
with its own instructions for handling the various peripherals and presenting
them in a standardized way decided by the loader's designers. For example, an
HDD and an SSD both provide additional memory for the computer but operate very
differently (the HDD has moving mechanical parts, the SSD uses only electrical
signals). The loader's designers might decide that every device which
provides additional memory must have a method of reading a certain number of
bytes from the device, so they create two sets of instructions: one for reading
a certain number of bytes from an HDD, and another for reading a certain number
of bytes from an SSD.

Then when the programmers create their programs that want to read memory from a
peripheral, they don't have to think about what sort of peripheral is plugged in
and issue specific instructions for it. Instead, they include a special
...
computer programs can easily create programs that work for a wide variety of
peripherals.

The instructions used by the loader for communicating with a specific device are
called the device's **driver**. The special instructions that programs use to
tell the loader that they want to use a general type of device are called an
**interface**.
{: .definition}
...
it provides a wide variety of drivers for using many computer peripherals, and
it provides standardized interfaces for computer programmers to use those
drivers. It deserves a fancier name.

The main program run by a computer which manages the running of other programs
and coordinates the usage of computer peripherals is called the **kernel**.
{: .definition}
...
programs need to interact with.
